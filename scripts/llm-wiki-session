...
def utc_now() -> str:
    return (
        dt.datetime.now(UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    )
...
def digest_path(
    root: Path, harness: str, native_session_id: str, now: str | None = None
) -> Path:
...
def feedback_scope_hint(text: str, feedback_type: str) -> str:
    lower = text.lower()
    if re.search(
        r"\b(always|never|for future|next time|from now on|rule|policy)\b", lower
    ):
        return "global"
    if re.search(
        r"\b(repo|project|pr|release|docs?|readme|website|tests?|config|hook|environment|env)\b",
        lower,
    ):
        return "repo"
...
def feedback_lesson(feedback_type: str, preview: str) -> str:
    ...
    if feedback_type == "decision":
        return (
            f"User approved proceeding with the prior plan or decision point: {preview}"
        )
...
def classify_feedback_text(text: str, force: bool = False) -> dict[str, str] | None:
    ...
    if not force and re.fullmatch(
        r"(ok|okay|cool|thanks|thank you|ty|k|👍|👌)[.!]*", lower
    ):
        return None
...
def normalize_event(
    args: argparse.Namespace, payload: dict[str, Any], root: Path
) -> dict[str, Any]:
    now = utc_now()
    ...
    event = {
        ...
        "trigger": trigger,
        "payload_preview": compact_json(
            payload,
            max_chars=int(
                getattr(args, "max_event_bytes", MAX_EVENT_PREVIEW_CHARS)
                or MAX_EVENT_PREVIEW_CHARS
            ),
        ),
    }
    ...
    if transcript_path:
        event["transcript_path_hash"] = hashlib.sha256(
...
def update_state(
    root: Path, event: dict[str, Any], config: dict[str, Any]
    ...
    state["git_remote"] = git_value(
        event.get("cwd"), ["config", "--get", "remote.origin.url"]
    )
...
def build_feedback_candidate(
    ...
    feedback_cfg = (
        config.get("feedback") if isinstance(config.get("feedback"), dict) else {}
    )
    ...
    if not classification:
        return None
    if (
        classification["feedback_type"] == "approval"
        and feedback_cfg.get("capture_approvals", True) is False
        and not force
    ):
        return None
...
def maybe_record_feedback_candidate(
    ...
    state["feedback_candidate_count"] = (
        int(state.get("feedback_candidate_count") or 0) + 1
    )
    write_json(
        state_path(root, str(state["harness"]), str(state["native_session_id"])), state
    )
...
def is_tool_event(event: dict[str, Any]) -> bool:
    ...
    return bool(event.get("tool_name")) or name in {
        "PreToolUse",
        "PostToolUse",
        "PermissionRequest",
        "BeforeTool",
...
def should_write_digest(
    ...
    auto = (
        config.get("auto_capture")
        if isinstance(config.get("auto_capture"), dict)
        else {}
    )
    event_name = str(event.get("hook_event_name") or "")
    ...
    if (
        threshold > 0
        and is_tool_event(event)
        and tool_count > 0
        and tool_count % threshold == 0
    ):
...
- If the session ended mid-task, use `llm-wiki-session rehydrate --session-id {
        state.get("llm_wiki_session_id")
    }` before continuing.
""".strip()
...
def write_digest(root: Path, state: dict[str, Any], trigger: str) -> Path:
    path = digest_path(
        root,
        str(state["harness"]),
        str(state["native_session_id"]),
        str(state.get("last_seen_at") or utc_now()),
    )
...
def iter_feedback_candidates(root: Path) -> list[dict[str, Any]]:
    path = feedback_candidates_path(root)
    status = (
        read_json(feedback_status_path(root), {"promoted": {}})
        if feedback_status_path(root).exists()
        else {"promoted": {}}
    )
    promoted = (
        status.get("promoted") if isinstance(status.get("promoted"), dict) else {}
    )
...
def handle_event(args: argparse.Namespace, payload: dict[str, Any]) -> str:
    ...
    event_name = str(event.get("hook_event_name") or "")
    rehydrate_cfg = (
        config.get("rehydrate") if isinstance(config.get("rehydrate"), dict) else {}
    )
    context = ""
...
def manual_payload(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        or f"manual-{dt.datetime.now().strftime('%Y%m%d-%H%M%S')}",
...
def iter_states(root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted((root / "state").glob("*/*.json")):
        ...
        if isinstance(data, dict):
            states.append(data)
    return sorted(
        states, key=lambda item: str(item.get("last_seen_at") or ""), reverse=True
    )
...
def run_list(args: argparse.Namespace) -> int:
    ...
    if args.json:
        print(
            json.dumps(
                {"sessions_root": str(root), "sessions": states},
                indent=2,
                sort_keys=True,
            )
        )
...
def run_rehydrate(args: argparse.Namespace) -> int:
    root = sessions_dir(resolve_hub(args))
    context = build_rehydrate_context(
        root,
        cwd=args.cwd,
        session_id=args.session_id,
        topic=args.topic,
...
def topic_path(hub: Path, slug: str) -> Path:
    ...
    entry = (
        registry.get("wikis", {}).get(slug)
        if isinstance(registry.get("wikis"), dict)
        else None
    )
...
def append_topic_log(
    topic_root: Path, message: str, operation: str = "session"
) -> None:
...
def run_promote(args: argparse.Namespace) -> int:
    ...
    append_topic_log(
        topic_root,
        f"promoted session digest {state.get('llm_wiki_session_id')} → raw/notes/{filename}",
    )
    print(str(dest))
    return 0
...
def feedback_candidate_matches(
...
def run_feedback_list(args: argparse.Namespace) -> int:
    root = sessions_dir(resolve_hub(args))
    candidates = [
        c for c in iter_feedback_candidates(root) if feedback_candidate_matches(c, args)
    ]
    if args.json:
        print(
            json.dumps(
                {"sessions_root": str(root), "feedback_candidates": candidates},
                indent=2,
                sort_keys=True,
            )
        )
    ...
    print(
        markdown_table(
            [
                ...
                (
                    "Promoted",
                    json.dumps(candidate.get("promoted"))
...
def feedback_payload(args: argparse.Namespace) -> dict[str, Any]:
    return {
        "session_id": args.session_id
        or f"manual-feedback-{dt.datetime.now().strftime('%Y%m%d-%H%M%S')}",
...
def run_feedback_capture(args: argparse.Namespace) -> int:
    ...
    candidate = maybe_record_feedback_candidate(
        root, state, event, payload, config, force=True
    )
    rebuild_indexes(root)
    if not candidate:
...
def run_feedback_promote(args: argparse.Namespace) -> int:
    ...
    summary = (
        candidate.get("distilled_lesson") or f"Feedback candidate {candidate.get('id')}"
    )
    promoted = (
        textwrap.dedent(
            f"""
        ---
        title: {
                yaml_string("Feedback Candidate Promotion: " + str(candidate.get("id")))
            }
        source: {yaml_string(str(feedback_candidates_path(root)))}
        type: notes
        ingested: {today()}
        tags: [feedback, feedback-candidate, user-feedback, {
                yaml_string(str(candidate.get("feedback_type")))
            }, {yaml_string(args.topic)}]
        summary: {yaml_string(summary)}
        ---
    ...
    append_topic_log(
        topic_root,
        f"promoted feedback candidate {candidate.get('id')} → raw/notes/{filename}",
        operation="feedback",
    )
    print(str(dest))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="llm-wiki-session",
        description="Capture and rehydrate llm-wiki session context.",
    )
    parser.add_argument("--hub", help="Override wiki hub path.")
    parser.add_argument(
        "--local",
        action="store_true",
        help="Use .wiki/.sessions in the current directory.",
    )
    sub = parser.add_subparsers(dest="command", required=True)

    enable = sub.add_parser(
        "enable",
        help="Enable or reconfigure automated session capture in HUB/.sessions/config.json.",
    )
    enable.add_argument("--mode", choices=sorted(MODE_CONFIGS), default="balanced")
    enable.add_argument(
        "--tool-events",
        type=int,
        help="Override automated tool-event checkpoint threshold.",
    )
    enable.add_argument(
        "--force",
        action="store_true",
    ...
    hook = sub.add_parser(
        "hook", help="Record a harness hook event from JSON on stdin."
    )
    hook.add_argument(
        "--harness", help="Harness name, e.g. codex, claude, opencode, gemini."
    )
    ...
    hook.add_argument(
        "--if-enabled",
        action="store_true",
        help="Honor .sessions/config.json enabled=false; default is enabled when no config exists.",
    )
    hook.add_argument("--max-event-bytes", type=int, default=MAX_EVENT_PREVIEW_CHARS)
    hook.set_defaults(func=run_hook)

    capture = sub.add_parser(
        "capture", help="Force a manual session digest checkpoint."
    )
    capture.add_argument("--harness", default="manual")
    capture.add_argument(
        "--session-id", help="Native session id; defaults to manual timestamp."
    )
    capture.add_argument(
        "--cwd", help="Session working directory; defaults to current directory."
    )
    capture.add_argument(
        "--transcript-path",
        help="Transcript pointer to record without copying contents.",
    )
    ...
    rehydrate = sub.add_parser(
        "rehydrate", help="Print a compact context block from matching session digests."
    )
    rehydrate.add_argument(
        "--session-id", help="llm-wiki session id or native session id."
    )
    rehydrate.add_argument(
        "--cwd", help="Match sessions by cwd; defaults to current cwd."
    )
    ...
    promote = sub.add_parser(
        "promote", help="Promote a session digest into a topic raw note."
    )
    ...
    feedback = sub.add_parser(
        "feedback",
        help="List, capture, show, and promote curated user-feedback candidates.",
    )
    feedback_sub = feedback.add_subparsers(dest="feedback_command", required=True)

    feedback_list = feedback_sub.add_parser(
        "list", help="List feedback candidates captured from user turns."
    )
    feedback_list.add_argument(
        "--session-id", help="Filter by llm-wiki session id or native session id."
    )
    feedback_list.add_argument("--cwd", help="Filter by cwd.")
    feedback_list.add_argument(
        "--type",
        choices=["approval", "correction", "decision", "manual", "preference"],
    ...
    feedback_list.add_argument(
        "--unpromoted",
        action="store_true",
        help="Only show candidates not yet promoted.",
    )
    ...
    feedback_capture = feedback_sub.add_parser(
        "capture", help="Manually add a feedback candidate."
    )
    feedback_capture.add_argument(
        "--text",
        required=True,
        help="User feedback text to distill and store as a candidate.",
    )
    feedback_capture.add_argument("--harness", default="manual")
    feedback_capture.add_argument(
        "--session-id", help="Native session id; defaults to manual-feedback timestamp."
    )
    feedback_capture.add_argument(
        "--cwd", help="Session working directory; defaults to current directory."
    )
    ...
    feedback_promote = feedback_sub.add_parser(
        "promote", help="Promote a feedback candidate into a topic raw note."
    )
