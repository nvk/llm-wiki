...
if [ -f "$HERMES_PLUGIN/plugin.yaml" ]; then
  ...
  if python3 -c "import yaml,sys; d=yaml.safe_load(open('$HERMES_PLUGIN/plugin.yaml'));       assert d.get('name') and d.get('version') and isinstance(d.get('provides_hooks'),list) and d['provides_hooks'], 'missing name/version/provides_hooks'" 2>/dev/null; then
...
ok = 'name:' in t and 'version:' in t and 'provides_hooks:' in t
